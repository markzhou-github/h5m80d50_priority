...
def resolve_minute_feature_source(
    ...
    if bucket_dir:
        print(f"[minute-source] bucket_dir not found yet; path={bucket_dir}", flush=True)
        return bucket_dir

    if panel:
        print(f"[minute-source] panel not found yet; path={panel}", flush=True)
        return panel

    if legacy_dir:
        print(f"[minute-source] legacy_dir not found yet; path={legacy_dir}", flush=True)
        return legacy_dir

    print("[minute-source] disabled", flush=True)
    return None
