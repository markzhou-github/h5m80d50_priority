...
def process_one_stock(
    ...
    try:
        output_path = args.out_dir / f"{ts_code}.parquet"
        output_file_existed = output_path.exists()

        # A stock with no per-stock output file must be downloaded and saved,
        # even when --skip-if-complete is enabled. read_existing_1min_stock()
        # may still find legacy yearly files; those are merged as before, but
        # the requested single-file parquet is newly created.
        if output_file_existed and args.skip_if_complete and existing_covers_window(
            args.out_dir,
            ts_code,
            start_dt,
            end_dt,
        ...
        if old.empty:
            if output_file_existed:
                print(f"[{ts_code}] existing output file contains no usable data.", flush=True)
            else:
                print(f"[{ts_code}] output file does not exist; it will be created.", flush=True)
        else:
        ...
        status = "updated" if output_file_existed else "created"
        print(
            f"[{ts_code}] {status} rows={summary['rows']:,} "
            f"new={new_only:,} overlap={overlap:,} -> {path}",
            flush=True,
        )
        time.sleep(args.sleep_sec + random.uniform(0.0, 0.2))
        return {"ts_code": ts_code, "status": status, "new_rows": new_only, "overlap_rows": overlap, **summary}
