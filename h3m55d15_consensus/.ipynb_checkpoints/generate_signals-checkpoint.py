...
def generate_for_date(
    ...
    *, historical_scoring: bool = False,
    ...
    risk_input = pd.DataFrame([row])[context.risk_features]
    print("[RISK FEATURES]")
    print(risk_input.T.to_string())
    risk = float(context.risk_model.predict_proba(risk_input)[:, 1][0])
  ...
  #  risk = float(context.risk_model.predict_proba(pd.DataFrame([row])[context.risk_features])[:, 1][0])
    ...
    print(f"risk = {risk}")
    print(f"threshold50 = {threshold50}")
    print(f"threshold60 = {threshold60}")
    print(f"keep50 = {risk <= threshold50}")
    print(f"keep60 = {risk <= threshold60}")
...
def main() -> None:
    ...
    old = history.loc[history.trade_date.eq(date),"bad_risk_logit"]

    print(f"[RISK CHECK] date={date}")
    print(f"old saved risk: {old.tolist()}")
    print(f"new risk: {diagnostic['bad_day_risk']}")

    if len(old):
        print(
            f"difference: "
            f"{diagnostic['bad_day_risk'] - old.iloc[0]}"
        )
